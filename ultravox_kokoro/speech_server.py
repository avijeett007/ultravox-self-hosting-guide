        ...
        while True:
            try:
                ...
                if "bytes" in message:
                    ...
                    # Check for speech while AI is speaking
                    if assistant.is_speaking and assistant.is_speech(audio_chunk):
                        print("User started speaking - interrupting AI...")
                        assistant.should_interrupt = True
                        # Wait a brief moment for current audio chunk to finish
                    ...
                    if assistant.add_audio(audio_chunk):
                        ...
                        assistant.should_interrupt = False
                        
                        # Split response into smaller chunks for interruptible playback
                        audio_segments = []
                        ...
                        if audio_segments:
                            for segment in audio_segments:
                                if assistant.should_interrupt:
                                    print("Interrupting AI speech...")
                                    break
                                    
                                # Convert and send audio chunk
                                audio_int16 = (segment * 32768).astype(np.int16)
                                await websocket.send_bytes(audio_int16.tobytes())
                        ...
                        if not assistant.should_interrupt:
                            await websocket.send_json({
                                "type": "state",
                                "state": "listening",
                                "timestamp": int(time.time() * 1000)
